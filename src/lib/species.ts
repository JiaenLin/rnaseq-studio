...
/**
 * The species a bundle says it is.
 *
 * rnaseq-studio records it as free text in meta.json — "Mus musculus", "mouse",
 * "Homo sapiens (GRCh38)" — because it comes from whatever the lab typed. That
 * is still far better evidence than guessing from gene names, so it is read
 * first and detection is only the fallback for a bundle that left it blank or
 * wrote something nobody can map.
 */
export function speciesOfMeta(text: string | undefined): Species | null {
  const t = (text ?? '').toLowerCase()
  if (!t || /unknown/.test(t)) return null
  if (/mus musculus|\bmouse\b|\bmus\b|\bmm(10|39)\b|grcm/.test(t)) return 'mouse'
  if (/homo sapiens|\bhuman\b|\bhs\b|\bhg(19|38)\b|grch/.test(t)) return 'human'
  return null
}
